...
login-problem-premium = Uchan Music は Spotify Premium でストリーミングしますが、このアカウントは Premium プランに加入していません。続けるには Premium アカウントでサインインしてください。
...
login-usage-consent = Uchan Music の利用者数の推計にご協力ください。
...
login-account-detail = このセッションは複数の Google アカウントにサインインしています。Uchan Music が使うアカウントを選んでください。
...
settings-saver-detail = Uchan Music が非アクティブの間アニメーションのフレームレートを制限する。次回起動時から適用
...
settings-close-to-tray-detail = ウィンドウを閉じたあとも Uchan Music をトレイに残して再生を続ける
...
settings-discord-buttons-detail = ステータスの下にフレンドが開けるリンクを置く。ひとつは提供元サービスの曲へ、もうひとつは Uchan Music へ
settings-discord-buttons-pick = ボタンを選ぶ
# the Discord status when the track is left out of it
discord-listening = 音楽を再生中
get-sonora = Uchan Music を入手
...
settings-version-detail = 実行中の Uchan Music のビルド
...
settings-notice = Copyright © 2026 Uchan Music Contributors. Uchan Music は完全に無保証です。フリーソフトウェアであり、GNU General Public License バージョン 3 以降の条件のもとで自由に再配布できます。Uchan Music は非公式であり、Spotify AB とは無関係です。
...
update-available = Uchan Music { $version } が公開されました
update-detail = 現在のバージョンは { $running } です。変更点を読むか、今すぐ更新できます。
update-detail-notes = 現在のバージョンは { $running } です。変更点を読んでから、インストールした方法で Uchan Music を更新してください。
update-notes = 変更点
update-now = 更新
update-later = 後で
...
settings-check-updates-detail = 起動時に一度、新しいバージョンがあるか GitHub に確認する。Uchan Music が自動で更新するのは Windows のみで、他の環境では変更点を案内する
settings-log = ログファイル
settings-log-detail = Uchan Music が実行中に書き出した内容。バグ報告に添付してください
