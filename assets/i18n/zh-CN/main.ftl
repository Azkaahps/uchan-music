...
login-problem-network = Uchan Music 无法连接到 Spotify。请检查网络连接并重试。
login-problem-cancelled = 你在批准登录之前关闭了浏览器页面。请重新开始以完成登录。
login-problem-refused = Spotify 拒绝了登录请求。请稍等片刻后重试。
login-problem-premium = Uchan Music 通过 Spotify Premium 串流，而此账户没有 Premium。请使用 Premium 账户登录以继续。
...
login-usage-consent = 帮助我们估算有多少人使用 Uchan Music。
...
login-account-detail = 此会话登录了多个 Google 账户。请选择 Uchan Music 应使用的账户。
...
settings-saver = 省电模式
settings-saver-detail = 当 Uchan Music 未聚焦时限制动画帧率，从下次启动起生效
settings-corners = 圆角
...
settings-close-to-tray-detail = 窗口关闭后将 Uchan Music 保留在系统托盘中并继续播放
...
settings-version = 版本
settings-version-detail = 你正在运行的 Uchan Music 版本
...
settings-notice = 版权所有 © 2026 Uchan Music 贡献者。Uchan Music 不提供任何担保。它是自由软件，你可以根据 GNU 通用公共许可证第 3 版或更高版本的条款重新分发。Uchan Music 是非官方项目，与 Spotify AB 无关联。
...
update-available = Uchan Music { $version } 已发布
update-detail = 你当前使用的是 { $running }。查看更新内容，或立即更新。
update-detail-notes = 你当前使用的是 { $running }。查看更新内容，然后按你的安装方式更新 Uchan Music。
update-notes = 新内容
update-now = 更新
update-later = 稍后
...
settings-check-updates-detail = 启动时向 GitHub 查询一次是否有新版本。Uchan Music 仅在 Windows 上自行安装更新；其他平台会指引你查看更新内容
settings-log = 日志文件
settings-log-detail = Uchan Music 运行时写下的内容。请将其附在错误报告中
