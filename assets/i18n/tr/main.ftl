...
# tray menu
tray-show = Uchan Music'yı Göster
tray-play = Çal
tray-pause = Duraklat
...
login-problem-network = Uchan Music, Spotify'a ulaşamadı. İnternet bağlantını denetleyip yeniden dene.
...
login-problem-premium = Uchan Music, Spotify Premium üzerinden yayın yapar ve bu hesapta Premium yok. Sürdürmek için Premium bir hesapla oturum aç.
...
login-usage-consent = Uchan Music'yı kaç kişinin kullandığını tahmin etmemize yardımcı ol.
...
login-account-detail = Bu oturumda birden çok Google hesabı açık. Uchan Music'nın kullanacağı hesabı seç.
...
settings-saver-detail = Uchan Music odakta değilken animasyonların kare hızını sınırla, sonraki açılıştan başlayarak uygulanır
...
settings-startup-detail = Uchan Music'nın açılışta gösterdiği ekran
...
settings-language-detail = Uchan Music'nın arayüz genelinde kullandığı dil
...
settings-typeface-detail = Uchan Music'nın arayüz genelinde kullandığı yazı tipi
...
settings-close-to-tray-detail = Penceresi kapandıktan sonra Uchan Music'yı sistem tepsisinde tut ve çalmayı sürdür
...
settings-discord-buttons-detail = Durumun altında arkadaşlarının açabileceği bağlantılar: biri parçanın kendi servisine, biri Uchan Music'ya
...
get-sonora = Uchan Music'yı edin
...
settings-notice = Copyright © 2026 Uchan Music Contributors. Uchan Music kesinlikle hiçbir garanti vermez. Bu özgür bir yazılımdır ve GNU Genel Kamu Lisansı sürüm 3 ya da sonrasının koşulları altında yeniden dağıtabilirsin. Uchan Music resmi değildir ve Spotify AB ile bağlantısı yoktur.
...
update-available = Uchan Music { $version } çıktı
update-detail = { $running } sürümünü kullanıyorsun. Nelerin değiştiğini oku ya da şimdi güncelle.
update-detail-notes = { $running } sürümünü kullanıyorsun. Nelerin değiştiğini oku, sonra Uchan Music'yı kurduğun yoldan güncelle.
...
settings-check-updates-detail = Açılışta GitHub'a bir kez daha yeni bir sürüm çıktı mı diye sor. Uchan Music güncellemeyi yalnızca Windows'ta kendi kurar; diğer yerlerde nelerin değiştiğini gösterir
settings-log = Günlük dosyası
settings-log-detail = Uchan Music'nın çalışırken yazdıkları. Hata raporuna ekleyin
