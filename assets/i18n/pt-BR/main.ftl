...
login-problem-network = O Uchan Music não conseguiu acessar o Spotify. Verifique sua conexão com a internet e tente novamente.
...
login-problem-premium = O Uchan Music usa o Spotify Premium para reproduzir, e esta conta não possui o plano. Faça login com uma conta Premium para continuar.
...
login-usage-consent = Ajude-nos a estimar quantas pessoas usam o Uchan Music.
...
login-account-detail = Esta sessão está conectada a mais de uma conta Google. Escolha qual o Uchan Music deve usar.
...
settings-saver-detail = Limitar a taxa de quadros das animações quando o Uchan Music não está em foco, aplicado a partir da próxima inicialização
...
settings-startup-detail = A tela que o Uchan Music abre ao iniciar
...
settings-language-detail = O idioma usado pela interface do Uchan Music
settings-language-system = Sistema
settings-language-search = Pesquisar idioma
settings-language-none = Nenhum idioma encontrado
settings-typeface = Fonte
settings-typeface-detail = A fonte usada pelo Uchan Music na interface
...
settings-close-to-tray-detail = Manter o Uchan Music na área de notificação e continuar tocando depois que a janela fechar
...
settings-discord-buttons-detail = Links abaixo do status para seus amigos abrirem, um para a faixa no serviço dela e outro para o Uchan Music
...
get-sonora = Baixar o Uchan Music
...
settings-notice = Copyright © 2026 Contribuidores do Uchan Music. O Uchan Music é distribuído com ABSOLUTAMENTE NENHUMA GARANTIA. É um software livre, e você pode redistribuí-lo sob os termos da GNU General Public License versão 3 ou posterior. O Uchan Music não é oficial e não é afiliado ao Spotify AB.
...
update-available = Uchan Music { $version } está disponível
update-detail = Você está na versão { $running }. Veja o que mudou ou atualize agora.
update-detail-notes = Você está na versão { $running }. Veja o que mudou e atualize o Uchan Music da forma como você o instalou.
...
settings-check-updates-detail = Perguntar ao GitHub na inicialização se uma nova versão está disponível. O Uchan Music instala a atualização sozinho apenas no Windows; em outras plataformas ele te direciona ao que mudou
settings-log = Arquivo de log
settings-log-detail = O que o Uchan Music escreveu enquanto rodava. Anexe-o a um relatório de bug
